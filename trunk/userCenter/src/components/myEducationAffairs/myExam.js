...
import { Row, Col, Icon, Input, Table, Button, Checkbox } from 'antd';

import styles from './index.css';


const myExam = ({ dispatch }) => {
  ...
  const columns = [{
    ...
    width: 170,
    render: (text, record) => (
      record.first < 60 ? <Checkbox className={styles.check} checked></ Checkbox> : <span></span>
    )
    ...
    width: 170,
    render: (text, record) => (
      record.second < 60 ? <Checkbox className={styles.check} checked></ Checkbox> : <span></span>
    )
  },{
    title: '2013年4月',
    dataIndex: 'third',
    ...
    width: 170,
    render: (text, record) => (
      record.third < 60 ? <Checkbox className={styles.check} checked></ Checkbox> : <span></span>
    )
  },{
    title: '2013年10月',
    dataIndex: 'fourth',
    ...
    width: 170,
    render: (text, record) => (
      record.fourth < 60 ? <Checkbox className={styles.check} checked></ Checkbox> : <span></span>
    )
  },
  {
    title: '总计',
