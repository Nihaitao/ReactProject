...
const myBook = ({ dispatch }) => {
  ...
  const columns = [{
    ...
    width: 170,
    render: (text, record) => (
      record.first < 60 ? <span><Icon type="check" className={styles.check} /></span> : <span></span>
    )
    ...
    width: 170,
    render: (text, record) => (
      record.second < 60 ? <span><Icon type="check" className={styles.check} /></span> : <span></span>
    )
  },{
    title: '2013年4月',
    dataIndex: 'third',
    ...
    width: 170,
    render: (text, record) => (
      record.third < 60 ? <span><Icon type="check" className={styles.check} /></span> : <span></span>
    )
  },{
    title: '2013年10月',
    dataIndex: 'fourth',
    ...
    width: 170,
    render: (text, record) => (
      record.fourth < 60 ? <span><Icon type="check" className={styles.check} /></span> : <span></span>
    )
  },
  {
    title: '总计',
...
function footer() {
  ...
  const listHtml = <Table bordered columns={columns} dataSource={data} pagination={false} scroll={{ x: 1150 }} footer={() => footer()} />
